...
export default function AdminPage() {
    ...
    if (!admin) return <main className="flex min-h-screen items-center justify-center bg-[#f5f6f3] px-6 text-[#17231e] swiss-grid"><form onSubmit={login} className="w-full max-w-md rounded-2xl border border-[#163b2d]/15 bg-white p-8 shadow-xl"><div className="mb-8 flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#163b2d] p-2"><img src="/logo.svg.png.png" alt="Swiss Dev logo" className="h-full w-full object-contain" /></span><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[#68776f]">Swiss Dev</p><h1 className="text-2xl font-semibold">Admin access</h1></div></div><div className="space-y-4"><Field label="Email" type="email" value={credentials.email} onChange={(event) => setCredentials({ ...credentials, email: event.target.value })} /><Field label="Password" type="password" value={credentials.password} onChange={(event) => setCredentials({ ...credentials, password: event.target.value })} /></div>{error && <p role="alert" className="mt-4 text-sm text-red-600">{error}</p>}<button className="mt-6 w-full rounded-lg bg-[#163b2d] py-3 text-sm font-semibold text-white">Sign in</button></form></main>;

    if (admin.mustChangePassword) return <main className="flex min-h-screen items-center justify-center bg-[#f5f6f3] px-6 text-[#17231e] swiss-grid"><form onSubmit={(event) => { event.preventDefault(); action({ action: "change-password", password: newPassword }); }} className="w-full max-w-md rounded-2xl border border-[#163b2d]/15 bg-white p-8 shadow-xl"><img src="/logo.svg.png.png" alt="Swiss Dev logo" className="mb-5 h-12 w-12 object-contain" /><h1 className="text-2xl font-semibold">Change your admin password</h1><p className="mt-2 text-sm text-[#68776f]">A new password is required before accessing the control room.</p><input required minLength={10} type="password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} placeholder="New password" className="mt-6 w-full rounded-lg border border-[#163b2d]/15 px-3 py-3 text-sm" />{error && <p role="alert" className="mt-3 text-sm text-red-600">{error}</p>}<button className="mt-4 w-full rounded-lg bg-[#163b2d] py-3 text-sm font-semibold text-white">Update password</button></form></main>;
